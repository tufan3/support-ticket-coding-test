...
<div class="container mt-5">
    ...
    <table class="table table-bordered table-striped" id="mytable">
        ...
        <tbody>
            <?php $sl = 0;
            if (!empty($tickets)): ?>
                <?php foreach ($tickets as $ticket):  $sl++;
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <?php if ($ticket['status'] == 'open') { ?>
                                ...
                                <a href="#" class="btn btn-danger btn-sm" onclick="close_ticket('<?php echo $ticket['id']; ?>',<?php echo $ticket['customer_id']; ?>)">Closed</a>
                            <?php } else { ?>
                                <a class="btn btn-secondary  btn-sm" disabled>Closed</a>
                            <?php } ?>
                        </td>
                    </tr>
                <?php endforeach; ?>
...
<script>
    ...
    // tickets closed///
    function close_ticket(closed_id, customer_id) {
        var datastr = "closed_id=" + closed_id + "&customer_id=" + customer_id;
        //   alert(datastr);
        if (confirm("Are you sure want to closed this issues?")) {
            $.ajax({
                type: "POST",
